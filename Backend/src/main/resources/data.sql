...
CREATE TABLE PRODUCTS (
    ID BIGINT NOT NULL, CATEGORY_ID BIGINT, TITLE VARCHAR(255), PRICE BIGINT, IMAGE VARCHAR(255), PRIMARY KEY (ID), Foreign Key FK_PRODUCTS_CATEGORIES (CATEGORY_ID) REFERENCES CATEGORIES (ID)
);

SHOW CREATE TABLE PRODUCTS;

CREATE TABLE TRANSACTIONS (
    ID BIGINT NOT NULL, TRANSACTION_DATE DATETIME, TOTAL_AMOUNT BIGINT, TOTAL_PAY BIGINT, PRIMARY KEY (ID)
);

SHOW CREATE TABLE TRANSACTIONS;

CREATE TABLE TRANSACTION_DETAILS (
    ID BIGINT NOT NULL, TRANSACTION_ID BIGINT, PRODUCT_ID BIGINT, QUANTITY BIGINT, SUBTOTAL BIGINT, PRIMARY KEY (ID), Foreign Key FK_TRANSACTION_DETAILS_TRANSACTION (TRANSACTION_ID) REFERENCES TRANSACTIONS (ID), Foreign Key FK_TRANSACTION_DETAILS_PRODUCTS (PRODUCT_ID) REFERENCES PRODUCTS (ID)
)
